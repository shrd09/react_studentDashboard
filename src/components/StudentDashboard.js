...
const StudentDashboard = ({ user ,handleLogout}) => {
  ...
  const handleShowMarks = async (courseId) => {
    try {
      const response = await fetch(`http://localhost:3000/courses/${courseId}/marks`, {
        headers: {
          'Authorization': `Bearer ${token}`,
        },
      });
  ...
  return (
    <Grid container spacing={2}>
      ...
      {/* marks in the List component */}
      {/* <Grid item xs={12}>
        <Typography variant="h6" gutterBottom>Your Marks:</Typography>
        <List>
